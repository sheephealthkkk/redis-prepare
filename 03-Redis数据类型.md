...
### 0.7 内存对齐：为什么结构体实际比肉眼算的大:rocket::rocket::rocket::rocket:
...
### 1.1 高层视角：String 就是 Redis 版的 `Map<String, Object>`:rocket:
...
### 1.5 RedisObject：String 外层的"包装盒":rocket::rocket:
...
**关键理解：type vs encoding 是解耦的**:rocket::rocket::rocket:
...
| CPU 缓存 | **连续内存，cache line 友好**:rocket: | 两块内存可能相距很远，cache miss |
...
把 **CPU 缓存**想象成你的书桌，**主内存**是墙边的大书架。

- 你不可能每次需要一本书，就起身去书架只拿那一本。
- 实际做法是：你一次搬来**一小排相邻的书**（比如10本），放在书桌的**一个书档**里。
- 这个“书档”就是 **缓存行**，这一小排书的大小是固定的（常见 **64 字节**）。

当你频繁翻看这10本书里的任何一本，都能从桌上快速拿到（缓存命中）。只有要看完全不在桌上的书时，才会再跑一趟书架，并把那一小排整排换上来。

---

### 2. 在 CPU 里的实际工作方式

- CPU 要读内存地址 `0x1000` 的一个 `int`（4字节）。
- 硬件会自动把地址 `0x1000` 所在的一整块区域（例如 `0x1000` ~ `0x103F`，共 64 字节）**整体加载到某个缓存行**。
- 接下来的访问如果落在这个 64 字节范围内，就是极快的 L1/L2 缓存命中，不再需要读内存。
- 这就是**空间局部性**的硬件基础：你访问了数组 `arr[0]`，`arr[1]` 大概率已在同一个缓存行内，不额外耗时。

**关键参数：**

- **大小**：目前 x86/ARM 主流 CPU 几乎都是 **64 字节**。
- **状态**：每个缓存行有 MESI 等一致性协议的状态（Modified, Exclusive, Shared, Invalid），用于多核同步。

---

### 3. 它和并发编程的痛点：伪共享:rocket::rocket::rocket::rocket::rocket:

这是 Cache Line 最“坑”的地方，也是最高性能优化的要点。

**场景：**
你有两个线程，分别频繁更新两个不同的变量 `head` 和 `tail`。它们逻辑上毫不相干。
...
- `head` 和 `tail` 都是 8 字节，挨着定义，**极大概率落在同一个 64 字节的缓存行**里。
- 核心 A 修改 `head`，该缓存行在核心 A 变为 **Modified**。
- 核心 B 要修改 `tail`，但它的缓存行还是旧的。为了保证一致性，必须让核心 A 的缓存行失效并写回，核心 B 重新加载整行。
- 结果：`head` 和 `tail` 明明没有竞争关系，却因为**被绑在同一缓存行**，导致两个线程不停互相使对方缓存失效，性能严重下降。这就是 **False Sharing（伪共享）**。
...
class Queue {
    volatile long head;
    // 填充56字节（在Java 8 可以用 @Contended 注解，或手动加 long p1,p2...p7）
    volatile long tail;
}
```

前边提到的 **Disruptor 高性能队列**，内部大量运用缓存行填充来消除伪共享，这也是其性能鹤立鸡群的关键之一。

---

### 4. 和你之前聊的技术串起来

- **volatile 变量**：保证可见性的代价就是会引起缓存行在总线上的广播和失效，本质就是缓存行一致性协议在工作。
- **CPU 缓存与内存的同步**：正是以缓存行为粒度进行，它跟之前说的“内存屏障”紧密相关。
- **非阻塞、高并发结构**（如 CAS 操作频繁的场景），伪共享是偷性能的大头，优化对象就是缓存行。
...
| 维度             | 说明                                                    |
| ---------------- | ------------------------------------------------------- |
| **是什么**       | CPU cache 和主存之间交换数据的最小固定大小块（通常64B） |
| **为什么存在**   | 利用空间局部性，减少访问主存次数                        |
| **对程序的影响** | 好的：连续访问快；坏的：伪共享在多核下极大拖慢性能      |
| **编程反应**     | 将高频独立写的变量缓存行对齐，用填充或注解避免伪共享    |

理解了 Cache Line，再看一些框架的源码（如 `ConcurrentHashMap` 的计数器填充、`Disruptor` 的 `RingBuffer` 设计），就不再奇怪为什么那里多了一堆看似无用的 `long` 变量。
...
| 参数 | 含义 | 典型用途 |
|------|------|---------|
| `NX` | key 不存在才设置（Not eXists） | **分布式锁**：`SET lock:order uuid NX PX 30000`:rocket: |
| `XX` | key 存在才设置（eXists） | **更新已存在的 key**，防止误创建:rocket: |
| `GET` | 返回旧值（Redis 6.2+） | **原子地替换并获取旧值**，替代旧的 `GETSET`:rocket: |
| `EX seconds` | 秒级过期 | 缓存：`SET user:1 json EX 3600`:rocket: |
| `PX ms` | 毫秒级过期 | 分布式锁精确控制：`PX 30000`:rocket: |
| `KEEPTTL` | 保留原有 TTL（Redis 6.0+） | 更新 value 但不重置过期时间:rocket: |

**为什么 SET 要带这么多参数？** 为了**原子性**:::rocket:rocket::rocket:。在早期版本：
...
#### LTRIM —— 截断（实现"保留最近 N 条"的利器）:rocket::rocket:
