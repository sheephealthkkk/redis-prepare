...
### 0.3 struct：C 语言的"只有属性的 Class":rocket::rocket:
...
### 0.4 sizeof：测量一个类型占用多少字节:rocket:
...
### 0.7 邪恶👿内存对齐：让结构体实际比肉眼算的大:rocket::rocket::rocket::rocket:
...
**Java 程序员完全可以跳过这个问题**——Java 的 String 底层是 `char[]` 或 `byte[]`，长度由数组的 `.length` 决定，天然就是二进制安全的。**C 不是，所以 Redis 需要自研 SDS:rocket:。**

---

## 第一章：String（字符串）—— Redis 万物之基

### 1.1 使用指南：String 就是 Redis 版的 `Map<String, Object>`:rocket:
...
| 字段 | 字节 | 含义 | Java 类比 |
|------|------|------|----------|
| `len` | 1 | 当前字符串长度:rocket: | `str.length()` |
| `alloc` | 1 | 总容量-1（不含头不含`\0`）:rocket: | `ArrayList` 的 `elementData.length`（容量） |
| `flags` | 1 | 类型标记（低 3 位标识 TYPE_8） | 类似 `instanceof` 检查 |
| `buf[]` | 可变 | 实际数据 + 结尾自动加 `\0`:rocket: | `ArrayList` 的 `elementData[]` |
...
**特性二：杜绝缓冲区溢出（自动扩容）**:rocket::rocket:
...
### 1.5 RedisObject：redis数据结构外层的"包装盒":rocket::rocket:
...
Redis 用 C 写的，C 没有多态/RTTI。

Redis 之所以耗费额外的内存（`redisObject` 结构体本身占 16 字节）来包装所有数据，核心原因是：**Redis 需要为一个“随时可能被淘汰、共享、过期或转换编码”的动态数据，挂载一个“元数据大脑”。**:rocket::rocket::rocket::rocket::rocket::rocket:

我们可以把 `redisObject` 看作是每个 Redis 键值对中 **“值（Value）”的航母战斗群指挥中心**。它不仅仅告诉计算机“这是什么类型”，更承担了内存管理、淘汰策略和编码转换的绝对核心职责。:rocket:

所以 Redis 自己造了一个统一的"头部"来描述任意数据：
...
| `ptr` | 8 字节 | 指向实际数据的指针（64 位系统指针=8 字节） | **这就是 Java 的引用！**:rocket: |

**关键理解：type vs encoding 是解耦的**:rocket::rocket::rocket::rocket:
...
### 1.6 String 的三种内部编码![img](https://cdn.xiaolincoding.com/gh/xiaolincoder/redis/%E6%95%B0%E6%8D%AE%E7%B1%BB%E5%9E%8B/string%E7%BB%93%E6%9E%84.png)
...
**条件：** value 能解析为整数:rocket:，且不超过 `long`（64 位有符号）范围:rocket:。

**做法：** 不分配 SDS！**直接把整数值存在 `redisObject` 的 `ptr`:rocket::rocket::rocket::rocket::rocket::rocket::rocket: 字段里。**
...
#### 1.6.2 embstr 编码——对象和数据黏在一起的优化![img](https://cdn.xiaolincoding.com/gh/xiaolincoder/redis/%E6%95%B0%E6%8D%AE%E7%B1%BB%E5%9E%8B/embstr.png)

**条件：** value 是字符串:rocket:，长度 ≤ 44 字节:rocket:。
...
| 维度 | embstr | raw |
|------|--------|-----|
| malloc 次数 | 1 次:rocket: | 2 次（redisObject + SDS 分开分配） |
| free 次数 | 1 次（释放一个块即释放所有）:rocket: | 2 次 |
| CPU 缓存 | **连续内存，cache line 友好**:rocket: | 两块内存可能相距很远，容易cache miss |
| 修改操作 | **不支持原地修改**，一改就变 raw:rocket: | 支持原地修改（SDS 有自己的扩容空间） |
...
这个问题问得很准，刚好能把你之前理解的“缓存策略”拉到更底层的**硬件层面**。

**Cache Line（缓存行）是 CPU 缓存与主内存之间交换数据的最小单位。**

---

### Cache Line（缓存行）：硬核的“快递周转箱”:rocket:

- **它是什么**：CPU缓存（L1/L2/L3）与内存（RAM）之间**数据交换的最小物理单位**。固定大小，现代CPU基本是 **64字节**。
- **本质**：它是一个**容器**。无论CPU想读内存里的一个`int`（4字节）还是一个`long`（8字节），硬件都**不会**只取那几字节，而是把这变量**所在的连续64字节内存**，整体打包成一个“快递箱”（Cache Line），搬运到CPU的缓存里。
- **关键特性**：它在缓存里是实实在在占据物理空间的。你的L1缓存有32KB，意思就是能放下 `32KB / 64B = 512` 个这样的“箱子”。

### Cache Miss（缓存未命中）：抓狂的“库存查询结果”

- **它是什么**：CPU核心执行指令，需要读取某个内存地址的数据时，发起的一次**查询结果**。
- **两种结果**：
  - **Hit（命中）**：CPU在L1缓存里找到了包含该地址的那个“Cache Line箱子”，直接读数据，**极快**（约1纳秒）。
  - **Miss（未命中）**：CPU在L1没找到，不得不层层下探（L2 -> L3 -> 内存），**极慢**（内存访问约100纳秒，相差两个数量级）。:rocket::rocket:
- **本质**：它是一个**动作反馈**（缺货了），代表着CPU在等待数据时产生的**空泡（Stall）** 延迟。

---

### 两者的铁血关系（如何联动）

它们的关系是：**Cache Miss 的结果，就是触发一次 Cache Line 的搬运。**

- 当发生 **Cache Miss** 时，CPU的硬件预取器（Prefetcher）会发出指令：“给我把地址 `0x1000` 所在的整条 **Cache Line（64字节）** 搬过来！”
- 所以，**Miss 是“因”（你没找到），Line 是“果”的载体（搬过来的箱子）。**
...
- **高缓存命中率**：由于 `embstr` 的所有数据都集中在一个连续的内存块中，当 CPU 需要读取这个字符串时，只需将包含它的那一个 64 字节的缓存行加载到 L1 缓存即可。后续对该字符串 `redisObject` 或实际内容的任何访问，都可以在 L1 缓存中直接命中（Cache Hit），速度极快。
- **利用空间局部性**：`embstr` 完美利用了 CPU 缓存的“空间局部性”原理。CPU 在加载 `redisObject` 时，会顺便把紧随其后的字符串内容也一起加载到缓存中，实现了“一次加载，多次受益”。

#### `raw`：缓存不友好

- **易产生缓存未命中**：访问一个 `raw` 编码的字符串时，CPU 首先需要加载存储 `redisObject` 的缓存行。然后，为了访问实际的字符串内容，CPU 必须通过 `ptr` 指针去访问另一块**不连续**的内存地址。
- **多次加载开销**：这个“跳跃”访问会导致两种情况：
  1. **强制缓存未命中 (Compulsory Cache Miss)**：如果目标内存地址不在缓存中，CPU 必须再次发起内存访问，将新的缓存行加载进来，这个过程比从 L1 缓存读取数据要慢数十甚至上百倍。
  2. **缓存行污染**：即使目标数据在缓存中，这个额外的加载操作也会占用宝贵的缓存空间，可能挤掉其他有用的数据。

`embstr` 是 Redis 为高频使用的小字符串做的**极致优化**，它利用连续内存让`redisObject`和字符串数据**在一个缓存行内就完成全部访问**，最大化缓存命中率。`raw` 则为了支持**大字符串和修改操作**（`embstr` 是只读的，修改会转为 `raw`），牺牲了部分缓存性能。



### 结合你之前的“伪共享”再看（升华）

有了这两个概念，你之前问的**伪共享（False Sharing）**就能被精准定义了：

- 假设变量 **X** 和 **Y** 恰好在同一条 **Cache Line（同一个箱子）** 里。
- 核心A修改X（**Hit**，在自己的缓存里，极快）。
- 核心B修改Y。因为缓存一致性协议，B发现自己的这条Cache Line失效了，去读时发生 **Cache Miss**。
- **结论**：**Cache Line 是物理承载体，Cache Miss 是性能损耗的代价。** 伪共享的本质，就是**因为共享了同一条 Cache Line，导致本不该发生的 Cache Miss 频繁暴增。**

 优化的核心思路就是：**通过填充（Padding）修改数据结构，让热点变量独占一条 Cache Line，从而减少不必要的 Cache Miss。**



#### 1.6.3 raw 编码——大字符串:rocket:![img](https://cdn.xiaolincoding.com/gh/xiaolincoder/redis/%E6%95%B0%E6%8D%AE%E7%B1%BB%E5%9E%8B/raw.png)
...
### 1.7 embstr 的"只读"特性:rocket::rocket::rocket:

**embstr 是只读的——任何修改操作都会让它退化为 raw。**

为什么？因为你分配的是 64 字节的 jemalloc 块，**块已经封顶了，无法扩容。** 要对 SDS 做 APPEND/SETRANGE，必须扩容 → 但原块找不到相邻空闲空间 → 只能搬到一个新的大块 → 此过程要求 redisObject 和 SDS 分离。:rocket::rocket::rocket::rocket::rocket:
...
**为什么 INCR 是原子的？** 不是因为锁——是因为 **Redis 命令执行全程单线程**:rocket::rocket:。INCR 的"读→加→写"中间不可能插入别的命令。但如果你在 Java 代码中写成两个命令：
...
#### SET / GET —— 最常用，但参数很多:rocket::rocket::rocket::rocket::rocket:
...
| 参数 | 含义 | 典型用途 |
|------|------|---------|
| `NX`:rocket: | key 不存在才设置（Not eXists） | **分布式锁**：`SET lock:order uuid NX PX 30000`:rocket: |
| `XX`:rocket: | key 存在才设置（eXists） | **更新已存在的 key**，防止误创建:rocket: |
| `GET` | 返回旧值（Redis 6.2+） | **原子地替换并获取旧值**，替代旧的 `GETSET`:rocket: |
| `EX seconds` | 秒级过期 | 缓存：`SET user:1 json EX 3600`:rocket: |
| `PX ms` | 毫秒级过期 | 分布式锁精确控制：`PX 30000` |
| `KEEPTTL` | 保留原有 TTL（Redis 6.0+） | 更新 value 但不重置过期时间:rocket::rocket::rocket::rocket::rocket::rocket::rocket: |

**为什么 SET 要带这么多参数？** 为了**原子性**:rocket::rocket:。在早期版本：
...
#### MSET / MGET —— 批量操作，减少 RTT:rocket::rocket::rocket:
...
#### INCR / DECR 家族 —— 原子的计数器:rocket::rocket:
...
#### GETRANGE / SETRANGE —— 把 String 当字节数组用:rocket::rocket:
...
#### GETBIT / SETBIT / BITCOUNT —— String 当位图用:rocket::rocket::rocket::rocket:
...
**选择策略：**:rocket::rocket:大部分字段一起读 → String JSON；

​		:rocket::rocket:经常只更新个别字段（如只改昵称不改头像）→ Hash；

​		:rocket::rocket:有嵌套结构（如 `user.orders[0].items`）→ 必须 String JSON。



redis里面string类型怎么存json，json不是有层级吗，string没有层级啊?

你这个问题特别有意思，点到了一个很容易混淆的地方：“Redis 的 String 类型没有层级，而 JSON 有层级，那怎么存得进去呢？”

答案其实很简单：**存进去的不是“层级结构”，而是“JSON 文本”本身。**

---

### 1. 核心理解：存的是一整串字符串

Redis 的 String 类型是 **二进制安全的**，最大可存 512 MB 的字节流。你给它什么字节，它就原样存什么字节。

JSON 虽然有层级，但它本身是一种**文本格式**。比如：

```json
{"name":"Alice","age":30,"address":{"city":"Paris"}}
```

它在内存里就是一段扁平的字符序列。你把这段字符序列当成一个普通字符串，用 `SET` 命令存进去就行了：

```bash
SET user:1001 '{"name":"Alice","age":30,"address":{"city":"Paris"}}'
```

Redis 完全不关心这个字符串内部是 `{` 还是 `[`，它只当它是 “一堆字节”。层级结构是你的应用程序在写入前**序列化**时产生的，也是读取后**反序列化**时解释的。

---

### 2. 完整流程：序列化与反序列化

你的程序里通常会这样操作（以 Python 为例）：
...
# 1. 把一个有层级关系的字典对象序列化成 JSON 字符串
user = {
    "name": "Alice",
    "age": 30,
    "address": {"city": "Paris"}
}
json_str = json.dumps(user)  # 变成字符串: '{"name":"Alice",...}'

# 2. 将这个 JSON 字符串存入 Redis String
r.set("user:1001", json_str)

# 3. 取出时，再把字符串反序列化回字典
data = r.get("user:1001")
user_obj = json.loads(data)   # 重新变成有层级的字典
print(user_obj["address"]["city"])  # 输出: Paris
```

所以 **“层级”是在应用程序侧通过 JSON 编码/解码实现的**，Redis 只负责托管中间那个扁平的字符串。

---

### 3. 那为什么我们不直接用 Redis 的 Hash 存结构化数据呢？

你可能会问：JSON 存成 String，和用 Hash 类型存字段，有什么区别？

| 方式               | 优点                                                         | 缺点                                                         |
| ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **String 存 JSON** | 1. 结构灵活，可随意嵌套<br>2. 一次性读写整个对象，原子性好   | 1. 不能直接读写单个字段（必须整取整存）<br>2. 修改一个字段需要全量序列化/反序列化<br>3. 无法对内部字段单独做 INCR、过期等操作 |
| **Hash 存字段**    | 1. 可单独读写、删除某个字段<br>2. 节省内存（尤其字段较短时）<br>3. 可对某个数值字段做 HINCRBY | 1. 不支持深层嵌套（只能存简单键值对）<br>2. 不能直接存数组等复杂结构<br>3. 结构不够灵活，字段固定 |

如果你的 JSON 只是简单的一层键值对，且经常需要修改单个字段，用 Hash 更好。  
如果你的对象嵌套很深、结构多变，或者就是直接当文档存，那用 String 存 JSON 是最常见的选择。很多 Redis 中间件（如 Spring Cache、某些 ORM 缓存）就是这样做的。

---

### 总结

- Redis 的 String 存 JSON，本质上存的是 JSON 的**文本表示**，没有任何结构上的冲突。
- 层级关系是应用程序通过 JSON 库在序列化和反序列化中处理的，Redis 完全不参与。
- 选择 String+JSON 还是 Hash，取决于你是否需要原子操作整个对象，还是要单独操作字段。

现在应该清晰了：**“String 没有层级”没关系，因为 JSON 字符串本身就是扁平的，它代表的结构在客户端复原。** 就像你把一个多层文件夹压缩成 `.zip` 文件存进硬盘，硬盘只看到一堆字节，解压后文件夹层级又回来了。
...
#### 场景五：GETSET / SET...GET 实现简单锁:rocket:
...
答：不是 Redis 定的，是 **jemalloc 分配器的 64 字节块限制 + 结构体大小约束** 联立方程的结果：:rocket:
...
## 第二章：List（列表）—— 从 LinkedList 到 quicklist 的进化->设计初衷是队列和栈:rocket:
...
**ziplist 的原理——把所有元素连续排在一起，用相对偏移:rocket::rocket::rocket::rocket::rocket:代替指针：**
...
**因为list的设计初衷就是队列和栈，所以它的数据结构设计成这样靠偏移量来找数据，也可以按照值找数据，但是这样就需要全表一个一个等值比较来查找**:rocket::rocket::rocket::rocket::rocket::rocket:
...
**prevlen 的编码规则（这是 ziplist 最大的坑）：**:rocket:
...
**现实生产中极少触发——** 所有 entry 整齐落在 253/254 边界本身就需要巧合。但理论上它存在，Redis 7.0 决定彻底根治。![image-20260624205115886](C:\Users\bkjysheep\AppData\Roaming\Typora\typora-user-images\image-20260624205115886.png)

**listpack 怎么解决的——把 `prevlen` 改成 `backlen`（存自身长度，不存前驱长度）：**:rocket::rocket::rocket::rocket:
...
### 2.5 quicklist——链表的中间地带quicklist = LinkedList<Ziplist>:rocket::rocket:

纯 ziplist/listpack 虽然省内存，但插入中间时需要搬移数据（O(N)）:rocket::rocket:；纯 linkedlist 虽然插入快，但指针开销大。**quicklist 取两者之长。**
...
#### BLPOP / BRPOP —— 阻塞弹出（消息队列的灵魂）:rocket::rocket:
...
#### LLEN —— O(1) 获取长度:rocket:

```bash
LLEN key        # 直接读 quicklist->count，O(1)，不遍历
```

#### LRANGE —— 获取范围内的元素:rocket:
...
#### LTRIM —— 截断（实现"保留最近 N 条"的利器）:rocket::rocket::rocket::rocket:
...
#### LINSERT —— 在指定元素前/后插入:rocket::rocket:
...
#### LMOVE / BLMOVE —— 跨队列移动（Redis 6.2+）:rocket::笑死了，这不是kafka的消费者应用转发嘛
...
**ACL（Access Control List，访问控制列表）** 是一种**基于列表的权限控制机制**，用来规定“谁（用户/进程）可以对某个资源做什么操作”。

它在很多地方出现：

- **文件系统**：Linux 的 `rwx` 权限，或者 Windows 的文件安全属性。
- **网络设备**：路由器/防火墙根据源 IP、目标端口等允许或拒绝流量。
- **数据库/中间件**：MySQL 的用户权限表，Redis 6.0 的用户权限系统。

核心模型就是：**主体（用户） → 资源 → 允许的操作**。

**ACK 是 Acknowledgement（确认）的缩写**，是通信中**接收方告诉发送方“我收到了”** 的机制，保证可靠传输。

### 最底层：TCP 协议中的 ACK

这是你经常见到的 ACK 源头。

- **机制**：TCP 是可靠传输，接收方每收到一个或多个数据包，就要回一个 **ACK 包**，告诉发送方“我已经成功收到序号到 X 的数据了”。
- **作用**：发送方如果在一定时间内没收到 ACK，就会**重传**。
- **和阻塞/非阻塞的关系**：非阻塞 socket 发送数据时，调用很快返回，但数据其实在内核缓冲区里，真正的可靠送达是靠后续的 TCP ACK 保证。

抓包时看到的 `[ACK]` 标志就是这个。

------

###  消息队列（MQ）中的 ACK（你最可能遇到的）

你在讲 Redis 缓存策略时提到了消息队列，MQ 里的 ACK 是用来**确认消费**的。

- **场景**：生产者发消息到队列，消费者取出消息处理。如果消费者刚取到消息就挂了，消息就丢了。所以需要一个 ACK 机制。
- **工作方式**：
  - 消费者取到消息后，消息变成“未确认”状态，不会从队列里删除。
  - 消费者处理完毕，**显式地发一个 ACK** 给队列。
  - a队列收到 ACK，才真正删除该消息。
  - 如果消费者断开连接或超时没有 ACK，队列会把消息**重新投递给其他消费者**。
- **典型例子**：
  - **RabbitMQ**：默认需要手动 `basicAck`，也可以自动 ACK。
  - **Kafka**：消费者通过**提交 offset（commit）** 来确认，这也是 ACK 的变体。
  - **Redis Stream**：也是用 `XACK` 命令来确认消息。:rocket:

这就是和之前 **“异步刷盘、写回”** 不一样的那一面：**处理完再确认，保证不丢消息**。

------

### 3. 其他常见的 ACK

- **硬件中断**：设备发中断，CPU 响应后回一个 ACK 告诉设备可以继续。
- **文件系统写入**：通过 `fsync` 等确保落盘后，内核或磁盘返回 ACK。
- **API/应用层**：一些同步接口里，服务端返回 `{"status":"ack"}` 表示收到请求，但处理结果后续再给（异步化）。



#### 场景二：最新动态 / 时间线 —— "LPUSH + LTRIM":rocket::rocket::rocket:
...
### 分页列表与浏览记录:rocket::rocket::rocket::rocket::rocket:

当数据量很大时，可以用 List 存储 ID 列表，实现快速分页查询。

**常用命令**

- `RPUSH article:list 1001 1002 1003` （顺序添加文章 ID）
- `LRANGE article:list 0 9` （第一页）
- `LRANGE article:list 10 19` （第二页）
- `LLEN article:list` 获取总条数，计算页数。

**场景**：

- **文章按发布顺序展示**：按时间顺序用 `RPUSH` 追加，列表本身就是有序的。
- **用户浏览记录**：每看一个商品，用 `LPUSH user:history:uid "sku:123"`，然后 `LTRIM` 只留最近 100 条。回头用 `LRANGE` 展示“最近浏览”。

**注意**：List 的分页是**基于索引的切片**，不是真正的“游标分页”，数据变动时可能重复或跳过，适合变动不频繁的场景。

### 有限集合的“最新 Top N”与淘汰:rocket::rocket::rocket::rocket:

**原理**：用 `LTRIM` 做固定大小的“裁剪”。

**命令**

- `LPUSH hot:articles "article:2001"`
- `LTRIM hot:articles 0 99` 永远只保留前 100 条。

**场景**：

- **排行榜**（只关心最新榜）：比如“最新上榜歌曲”、“最新爆款商品”，不是按热度排，是按加入时间排。
- **滑动窗口计数**：比如最近 1 分钟内登录的用户，把用户 ID 或时间戳推入 List，再用 `LTRIM` 或定时脚本剔除过期元素（也可结合 `ZSET` 做时间戳排序，但 List 最简单）。
...
答：LINDEX（按索引查找元素）——要先定位到是第几个 quicklistNode，再在那个节点的 ziplist 里线性查找:rocket::rocket:。O(N) 不变，但常数略大（两层遍历）。不过 Redis 定位是"内存操作快过网络 I/O"，这点开销在微秒级，相比网络往返可忽略。
...
### 3.4 dict 编码——Redis 的"哈希表引擎":超过512与64时演进
...
#### 3.5.1 为什么不能一次性 rehash？:rocket::rocket::rocket::rocket:
...
**如果没有任何请求怎么办？** Redis 的定时任务 `serverCron` 每 100ms 运行一次，每次用 1ms 时间片专门做 rehash 搬迁，确保空实例的 rehash 也会向前推进。:rocket::rocket::rocket::rocket:
...
**HSET 的复用性：** Redis 4.0 起 HSET 就可以接受多个 field-value 对，HMSET 已被废弃。HSET 返回的是**新增** field 数量（更新已有 field 不算新增）。:rocket::rocket::rocket::rocket:
...
**安全替代方案——HSCAN：**:rocket::rocket::rocket:

`HSCAN` 是 Redis 用于**增量式迭代（Incremental Iteration）** 哈希（Hash）类型中所有字段（Field）和值（Value）的命令。这个命令的核心价值在于，它能将一次可能造成长时间阻塞的大操作，分解成多次小操作，从而避免服务卡顿
...
- **数据可能有重复**：在遍历过程中，如果数据集发生变化（如新增或删除了元素），可能导致**同一个元素被返回多次**。因此，业务逻辑**必须支持幂等性**。
- **数据可能遗漏**：同理，在遍历过程中发生的数据变更，也可能导致**某些元素被遗漏**。
- **`COUNT` 并非精确分页**：`COUNT` 只是对服务端的**性能提示**，不能用作精确的分页大小。
- **`MATCH` 在服务端过滤**：模式匹配在服务端执行，虽减少了网络传输，但**依然会扫描全量数据**，只是过滤了返回结果。

```
HSCAN key cursor [MATCH pattern] [COUNT count] [NOVALUES]
```

`HSCAN` 命令从 Redis 2.8.0 版本开始可用。它支持以下参数：

- `key`：要迭代的哈希表的键名。
- `cursor`：游标。**首次调用必须传 `0`**，后续调用需传入上一次命令返回的新游标。
- `MATCH pattern`：**(可选)** 一个 glob 风格的匹配模式，用于过滤字段名。
- `COUNT count`：**(可选)** 提示 Redis 每次迭代应返回多少元素，**仅作为对服务端的提示（Hint），不保证精确**。**默认值为 `10`**。
- `NOVALUES`：**(可选，Redis 7.4+)** 如果指定了此选项，命令将**只返回字段名**，不返回对应的值。
...
#### HRANDFIELD —— 随机取 field（Redis 6.2+）:rocket:randfile
...
```
**核心优势：**
- 原子性：HSET 单个 field 天然原子:rocket::rocket:，不需要乐观锁（CAS）防并发
- 节省序列化：JSON 的序列化/反序列化的 CPU 开销 > 单纯字节拷贝:rocket::rocket::rocket:
...
#### 场景三：计数器分组 —— "Hash 减少内存碎片":rocket::rocket::rocket:
...
#### 场景四：用户签到（Hash + Bitmap 组合）:rocket:
...
一些需要频繁读取、偶尔更新的配置，用 Hash 缓存起来。

**常用命令**

```
HSET config:app time_out 30 max_conn 1000 log_level "debug"
HGET config:app time_out
HMGET config:app time_out max_conn   # 一次拿多个配置项
```
...
- 功能开关：`HSET feature:flags new_ui 1 beta_test 0`，可以在代码里直接 `HGET` 检查。
...
```
HSET relation:u1:u2 status "friend" remark "小明" intimacy 85
HINCRBY relation:u1:u2 intimacy 1    # 互动一次，亲密度+1
```

**场景**

- 即时通讯：好友备注、置顶标志、消息免打扰状态。
- 社交平台：对某人的信任度、最近互动次数等。

### 接口限流（固定窗口计数器）:rocket:

使用 Hash 配合 `HINCRBY` 和 `EXPIRE` 做简单的接口限流。

**原理**

- key：用户或 IP
- field：当前分钟的时间戳（或窗口标识）
- value：请求次数
...
```
# 用户1001访问接口/api/msg
HINCRBY rate:user:1001 1719600000 1   # 时间戳取整到分钟
EXPIRE rate:user:1001 120             # 让整个hash 2分钟后过期
```

然后检查 `HGET rate:user:1001 1719600000` 是否超过阈值。
（更专业的限流会用滑动窗口，但 Hash 用于简单计数完全够用）

### 缓存对象的部分字段，用于高频读写分离:rocket::rocket:

当一张数据库表字段很多，但只有部分字段是热点时，可以只缓存热点字段到 Hash，冷字段留着查库。

**示例**
商品表有 50 个字段，但列表页只展示“价格、标题、封面”。

```
HSET product:2001 title "手机" price 2999 cover "/img/2001.jpg"
# 列表页读 Hash，详情页再查库获得完整参数和技术规格
```

这就避免了把整个大 JSON 放到 Redis String 里浪费内存。
...
### 4.3 intset——Redis 最精致的整数压缩结构整数集合:rocket:
...
### 4.4 Set 的 dict 编码——空 value 技巧:rocket::rocket::rocket::rocket::rocket:
...
### 4.5 集合运算——Set 的杀手锏:rocket::rocket::rocket::rocket::rocket::rocket:

**SINTER（交集）内部算法：**:rocket::rocket::rocket::rocket:
...
#### SADD / SREM / SISMEMBER —— 增删查:rocket:
...
#### SRANDMEMBER / SPOP —— 随机取:rocket::rocket::rocket:
...
**SPOP vs SRANDMEMBER 的区别（重要）：**:rocket:
...
#### 集合运算（Set 的杀手锏）:rocket::rocket::rocket::rocket:
...
**SINTER / SUNION / SDIFF 的复杂度与风险：:rocket:**:rocket:
...
#### 场景一：点赞/收藏/已读 —— "Set = 天然的幂等去重":rocket::rocket::rocket:
...
#### 场景四：标签系统 —— "Set 天然支持标签的增删和交集检索":rocket::rocket::rocket:
...
**反向索引的威力：** 一个文章有多个标签，每个标签又索引到多篇文章 → SINTER 两个标签 = 找到满足两个标签的所有文章 → 比 SQL `WHERE tag IN ('a','b')` 更贴合多对多场景。:rocket::rocket::rocket::rocket::rocket:
...
```
SADD page:index:uv:20260624 user:1001 user:1002 user:1003
SCARD page:index:uv:20260624   # 当日 UV
```

还可以用 **HyperLogLog** 节省内存（牺牲一点点精度），但 Set 是绝对精确的，数据量不大时用 Set 更直观，还能看具体哪些用户来过。

### 黑白名单 / 权限控制:rocket::rocket:

**需求**：快速判断某个 IP 是否在黑名单里。
...
```
SADD blacklist:ip 10.0.0.5 10.0.0.8
SISMEMBER blacklist:ip 10.0.0.5   # 返回 1，说明在名单内
```

**场景**

- 防爬虫：把恶意 IP 加进 Set，网关拿到请求后 `SISMEMBER` 检查，O(1) 极快。
- 活动白名单：只有白名单里的用户才能参与活动。
...
**结论：** dict 负责"按名字查分数"（类似 HashMap），skiplist 负责"按分数排序和查排名"（类似 TreeMap）:rocket::rocket::rocket:。member 的 SDS 字符串在两个结构之间**共享同一份内存**，没有浪费。

---

### 1. Dict（字典）的作用：:rocket:O(1) 精准打击

Dict 是一个标准的哈希表，它的 **Key 是 ZSET 的成员（Member），Value 是该成员对应的分数（Score）**。

- **核心职责**：**“成员存在性检查”和“极速查分”**。
- **支撑的命令**：`ZSCORE`（O(1) 复杂度）、`ZADD` 时判断成员是否已存在（做去重）、`ZREM` 删除时快速定位。
- **为什么跳表替代不了它**：虽然跳表查找也是 O(log N)，但 Dict 是 O(1)。在高并发场景下，对于 `ZSCORE` 这种极高频操作，O(1) 的性能优势是巨大的。更关键的是，如果没有 Dict，在执行 `ZADD` 更新分数时，你要在跳表里遍历查找成员是否存在，这会大大增加时间复杂度。

### 2. 跳表（SkipList）的作用：:rocket:O(log N) 范围扫描

跳表存储的是**有序的**节点，它按 **（分数 Score，成员 Member）** 进行双重排序。

- **核心职责**：**维护顺序，支撑范围操作**。
- **支撑的命令**：`ZRANGE`（按排名取范围）、`ZREVRANGE`（按排名倒序取）、`ZRANK`（查排名）、`ZCOUNT`（统计分数区间内元素个数）、`ZREMRANGEBYSCORE`（按分数范围删除）。
- **为什么 Dict 替代不了它**：Dict 中的数据是**完全无序**的，它无法回答“排在第 10 名到第 20 名的是谁”或者“分数在 50 到 100 之间的有哪些”。只有跳表这种链式有序结构，才能以 O(log N) 定位到起点，然后顺着指针一路向后遍历。

### 3. 两者是如何配合工作的？（关键！）:rocket::rocket::rocket:

以 `ZADD` 命令为例（比如要插入一个已存在的成员，但修改它的分数），看看它们必须联动的逻辑：

1. **先去 Dict 里看一眼**：O(1) 检查成员是否存在。
2. **如果存在**：从 Dict 里**瞬间拿出旧的分数**。
3. **拿着旧分数去跳表**：因为跳表是按分数排序的，Redis 拿着“旧分数 + 成员名”去跳表里，**O(log N) 定位到该节点**，将其删除。
4. **更新 Dict**：把 Dict 里该成员对应的 Value 改写成新分数。
5. **插入跳表**：拿着“新分数 + 成员名”，在跳表中 O(log N) 找到新位置，插入进去。

**如果没有 Dict**：第三步中，为了在跳表里删除这个成员，Redis 不知道旧分数是多少，只能遍历跳表每个节点去比对成员名（字符串比较），复杂度会退化为 O(N)，且每次修改都极耗性能。

这个“分数”（Score）就是你在写入数据时，**自己指定并存入的一个“排序指标”**。它本质上是一个**双精度浮点数（double）**，用来决定成员（Member）在集合中的位置。

你可以把它理解为你给每个元素贴上的一个**“权重标签”**。你想让谁排在前面，就给谁一个更小的（或更大的）数字。

### 分数在实际业务中代表什么？

它非常灵活，完全由你的业务场景决定：

- **游戏排行榜**：分数 = 玩家的**总得分**（分数越高排名越靠前）。
- **消息队列（延迟队列）**：分数 = 任务的**执行时间戳**（如 `1736000000`）。消费者按分数从小到大取，分数最小的最先执行。
- **商品热度榜**：分数 = 商品的**点击量**或**综合评分**。
- **优先级任务**：分数 = 任务的**优先级数值**（如 1 为最高，10 为最低）。

### 5.2 跳表（skiplist）长什么样？![img](https://cdn.xiaolincoding.com/gh/xiaolincoder/redis/%E6%95%B0%E6%8D%AE%E7%B1%BB%E5%9E%8B/3%E5%B1%82%E8%B7%B3%E8%A1%A8-%E8%B7%A8%E5%BA%A6.drawio.png)
...
**关键理解二：** :rocket:`span` 是 Redis 跳表特有的设计——记录了"在这一层上，从当前节点跳到 forward 节点，中间跳过了多少个 level-0 的节点"。这是 ZRANK（查排名）能做到 O(logN) 的秘密所在。

---

### 5.4 用具体例子理解 span(跨度) 和 ZRANK
...
level 2 (最高层，只有3个节点)：
  HEAD ───────(span=3)───────► charlie(100) ───(span=2)───► NULL
        (跳到charlie需3步：alice,bob,charlie)   (到末尾需2步：dave,eve)

level 1 (中间层)：
  HEAD ────(span=2)────► bob(95) ──(span=1)──► charlie(100) ──(span=2)──► eve(110) ──► NULL
        (需2步：alice,bob)       (紧邻1步)        (需2步：dave,eve)

level 0 (最底层，完整链表，跨度恒为1)：
  HEAD ► alice(90) ► bob(95) ► charlie(100) ► dave(100) ► eve(110) ► NULL
       (span=1)    (span=1)    (span=1)      (span=1)    (span=1)
```

### 🚀 这个 `span` 到底是干嘛用的？（最核心的价值）

**`span` 的存在，是为了让 `ZRANK`（查排名）和 `ZRANGE`（按排名取元素）这两个命令达到 O(log N) 的时间复杂度，而不用去遍历底层链表！**

假如没有 `span`，你要查 `eve` 的排名，就得从 `alice` 开始，沿着 `level 0` 一个一个往下数，复杂度是 O(N)。

**有了 `span`，查找过程是这样的（以查 `dave` 的排名为例）：**

1. **第一跳**：从 `HEAD` 的 `level 2` 指向 `charlie`，`span = 3`。意味着这一跳跨越了 3 个节点，所以 `rank += 3`（当前排名记为 3）。
2. **向下走**：到 `level 1`，从 `charlie` 指向 `eve`，但 `dave` 在 `eve` 前面，且 `eve` 的 `span = 2` 指向末尾，所以不走这一步（否则会跳过目标）。
3. **继续下降**：到 `level 0`，从 `charlie` 指向 `dave`，`span = 1`，`rank += 1`，最终得到 `rank = 4`（即 `dave` 是第 4 个元素）。

**整个过程利用了跳表的层级跳跃，累加沿途的 `span`，瞬间就算出了排名，完全不需要去遍历底层链表。**
...
#### ZREM / ZSCORE / ZRANK / ZREVRANK:rocket:
