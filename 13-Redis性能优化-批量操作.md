...
### 1.4 Cluster 模式下 MGET/MSET 的"坑"——深度剖析
...
使用 Hash Tag 的 slot 计算：
  CRC16("{user}:1001") % 16384  ← 只对 { } 中的 "user" 算哈希
  CRC16("{user}:2002") % 16384  ← "user" 一样 → slot 一样！
  CRC16("{user}:3003") % 16384  ← "user" 一样 → slot 一样！
  
  → 三个 key 落在同一个 slot → 同一个节点 → MGET/MSET 可以用了！
```

```java
// ═══ 使用 Hash Tag 让多个 key 落在同一个 slot ═══

// ❌ 不用 Hash Tag（在 Cluster 中可能跨 slot）
List<String> vals = jedis.mget("user:1", "user:2", "user:3");
// → CROSSSLOT 错误！

// ✅ 使用 Hash Tag（所有 key 强制同一 slot）
List<String> vals = jedis.mget("{user}:1", "{user}:2", "{user}:3");
// → ["val1", "val2", "val3"]  正常工作！

// ✅ Hash Tag 可以出现在 key 的任何位置
jedis.mset("{user}:info:1001", "v1", "{user}:profile:1001", "v2");
// 两个 key 中的 {user} 都是 tag → 同一 slot
```
...
| **原子性** | MSET 是原子的 | 不保证原子 |
| **Cluster** | 只能同 slot | 同 slot 跨 slot 都可以（客户端路由） |
...
Lua 脚本是三种批量方案中**唯一可以在服务端做条件判断和变量赋值**的。
