...
>
> 如果用 2 个哨兵：
> - 一个哨兵挂了，剩 1 个无法构成多数派 → 僵死:o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
> - 两个哨兵之间网络不通 → 各自为政 → 谁都无法获得多数票
>
> **3 个哨兵是满足「多数派可用」的最小奇数**。公式本质是：:o::o::o::o::o:
>
> ```
> 容忍的故障数 = (N - 1) / 2
...
- 哨兵节点数必须是奇数且 ≥ 3——为多数派投票服务:o::o::o::o::o:
...
#### 任务1：每 10s 一次 INFO 命令（:rocket::rocket:拓扑发现:rocket::rocket:）:rocket:哨兵通过 INFO 获取的信息：<br/>:o::o::o::o::o::o::o:1. 当前拓扑：有哪些从库、状态如何<br/>2. replica 的复制偏移量（offset）<br/>3. master 的 replid
...
**通过 INFO 实现的自动发现：**:rocket::rocket::rocket::rocket::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
...
**面试追问：PING 一下没回就是挂了？:o:**:o::o::o::o::o::o::o::o::o::o::o:
...
Redis Sentinel 中的哨兵节点之间互相发现，**并不是让主节点充当“转发员”交换信息，而是把主节点当作一个“公告板（Pub/Sub 频道）”**，各个哨兵通过这个公告板广播自己的存在，同时侦听其他哨兵的广播。:rocket::o::o::o::o::o::o::o::o:
...
### 发现过程（触发方式）:o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
...
| 设计选择                      | 好处                                                         |
| ----------------------------- | ------------------------------------------------------------ |
| **通过主节点交换信息**        | 哨兵不需要提前知道其他哨兵的地址，只需知道主节点，极大地简化了网络配置。:o::o::o::o::o::o::o: |
| **利用 Redis 本身的 Pub/Sub** | 可靠性高，复用 Redis 连接，无需引入额外的服务发现组件（如 Zookeeper）。:o::o::o::o::o::o::o: |
| **主节点宕机也能用从节点**    | 哨兵也会监控从节点，并在从节点的相同频道上发布/订阅；即使主节点挂掉，哨兵之间依然可以通过从节点的 Pub/Sub 保持联系。 |

所以准确的模型是：:o::o::o::o::o::o:**主节点（或从节点）提供了一个共同的“会议室”（频道），哨兵们在里面自报家门、互认兄弟，之后私聊（建立直连）进行正式沟通。**这是一个典型的**发布-订阅式服务发现**模式，简单且健壮——不需要额外的服务注册中心。

---

### 对比学习

| 维度           | **Redis Sentinel**              | **RocketMQ**                                | **Kafka (传统)**                     |
| :------------- | :------------------------------ | :------------------------------------------ | :----------------------------------- |
| **发现媒介**   | Redis 主节点自身的 Pub/Sub 频道 | 无中心，NameServer 各自独立                 | ZooKeeper 集群 (或 KRaft)            |
| **通信模式**   | 对等 Gossip (发布/订阅)         | Broker → 所有 NameServer (主动注册)         | Broker ↔ ZooKeeper (心跳+Watcher)    |
| **节点间关系** | 哨兵之间对等，无需中心          | NameServer 之间无任何通信                   | 存在 Controller 角色，强 Leader      |
| **一致性模型** | 最终一致，可能短暂脑裂          | 最终一致 (每个 NameServer 视图可能短暂不同) | 强一致性 (ZooKeeper ZAB 或 Raft)     |
| **故障检测**   | 哨兵主观/客观下线投票           | NameServer 基于心跳超时剔除                 | ZooKeeper 临时节点 + Controller 监听 |
| **运维复杂度** | 低，复用 Redis 进程             | **极低**，NameServer 几乎不维护状态         | 高，需独立维护 ZooKeeper 集群        |
| **适合规模**   | 中等，哨兵数量通常 3-5          | 大规模、高吞吐，NameServer 可水平扩展       | 超大规模，强依赖协调服务稳定性       |
...
**Gossip 协议**，直译就是“流言蜚语”协议。它是一种去中心化的、最终一致性的信息传播机制。

简单说，它模仿了人类社会中流言的传播方式：一个人听到一个消息，然后随机告诉几个朋友，这些朋友再告诉各自的朋友。用不了多久，所有人都知道了。

### Gossip 的核心思想:o::o::o::o::o::o::o::o::o::o:

在一个分布式集群里，每个节点没有全局视角。但它会周期性地随机选择几个其他节点，和它们交换自己知道的信息。主要有三种模式：

- **Push（推）**：节点 A 把自己的最新状态推送给节点 B。
- **Pull（拉）**：节点 A 向节点 B 请求它最新的状态。
- **Push-Pull（推拉结合）**：节点 A 和节点 B 互相交换各自的信息，各自更新。这是最常用的方式。

通过这种反复的随机交换，整个集群的状态会像病毒一样扩散，最终所有节点达成一致。它的优点是**极其鲁棒**，没有单点，不害怕个别节点或网络故障；缺点是只能保证**最终一致**，信息同步有延迟。

### Redis Sentinel 里的 Gossip 式通信

你之前已经准确指出了 Sentinel 的发现机制：**利用主节点充当 Pub/Sub**。这其实就是 Gossip 思想的一个具体应用。

Sentinel 没有专门的中心服务器来交换信息，而是：

1. 每个 Sentinel 都向主节点的 `__sentinel__:hello` 频道**发布**自己的信息（IP、端口、当前纪元等）。
2. 同时，每个 Sentinel 也都**订阅**了所有主节点的这个频道。
3. 这样，任何一个 Sentinel 发的消息，都会被其他所有 Sentinel 收到。
...
>
> 反例：如果 quorum 个哨兵恰好都和主库发生了网络不通（但主库本身运行正常），那这些哨兵都会把主库标记为 SDOWN，然后投票通过 ODOWN，接着执行故障转移。:o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
>
...
**面试金句：** 「这是一个经典的分布式系统中的 trade-off：故障检测速度 vs 误判率。调小 down-after-milliseconds 可以减少故障恢复时间（MTTR），但会增加误切风险。内网环境建议 5000~10000ms。」:o::o::o::o::o::o:
...
- 即使网络分区，只有**包含多数哨兵的那个分区**才能选出 Leader。:o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
 ...
 **一个纪元 = 一轮选举**，选举成功则获得leader节点，失败则下一个纪元，:o::o::o::o::o:并且重新从头开始拉票:o::o::o::o::o:
...
一个普通的 Sentinel 不会无缘无故变成候选者，它需要同时满足以下**所有**条件：

1. :o:**发现了客观下线**：它自己（或通过其他 Sentinel 的广播）确认了主节点已被判定为 `ODOWN`。
2. **满足“参选资格”**：
   - 它没有被配置为 `SENTINEL MONITOR` 的 `quorum` 所禁止（没有特殊规则排除它）。
   - 它通常需要处于一个**多数派分区**中。**:o::o::o::o::o::o::o:少数派分区的哨兵，即使理论上能成为候选者，也永远无法赢得选举。因此，:o::o::o::o::o::o::o:系统设计上会直接禁止它们成为候选者，以避免无意义的尝试和潜在的脑裂风险。**
   - **决定发起选举**：Sentinel 确认 `ODOWN` 后，会进入一个选举状态，准备向其他 Sentinel 拉票，请求它们投票给自己成为 Leader。**这个“站出来参选”的动作，把它从普通 Sentinel 变成了候选者。**

**只有处于多数派分区、且已确认主节点客观下线的 Sentinel，才有资格（并实际可能）成为候选者。**:o::o::o::o::o::o::o::o::o::o::o:
...
- **法定人数 (`quorum`)**：指的是一次投票**需要获得多少张赞成票才能成功**。它是一个**门槛**。

- **多数节点**：指**整个 Sentinel 集群中，所有在线且可达的哨兵总数（或者说总配额）的一半以上**。

  - 这是根据你配置的监控节点的 `quorum` 参数算出来的一个绝对标准。
  - 计算公式：(配置的哨兵总数 / 2) + 1 或者说 “超过半数”。
  - 它是一个**固定的门槛**，不因候选者增多而改变。

  

1. **多数节点不是“候选者里的多数”，而是“所有在线节点里的多数”。**:o::o::o: 投票的不只是候选者，**所有在线的 Sentinel 都有投票权**。:o::o::o:
2. **选票不是只在候选者之间流转。** 候选者去向所有 Sentinel 拉票，包括那些没有参选的普通 Sentinel。
3. **所有节点都能投票”这个规则，是理想网络环境下的规则。当网络分区发生时，“所有节点”被物理分割成了两个无法通信的集合。少数派分区的节点，实际上只能向少数派节点拉票，永远凑不够多数票。**

**所有哨兵都可以投票,:o::o:实际上如果有分区也就是多数节点数量的分区的投票是有意义的:o::o:，也只有多数分区的候选者可以成为leader，需要满足获取了多数票，并且还要大于法定人数**



假设还是 5 个哨兵，但由于交换机故障，它们被割成了两个区：:o::o::o::o::o::o::o::o::o::o::o::o::o:

- **分区 A**：哨兵 1, 2, 3
- **分区 B**：哨兵 4, 5

分区 A 里有 3 个哨兵，它们都检测到主节点挂了，都成了候选者。

- **分区 A 的候选者数量** = **3 个**。
- **但分区 A 看到的“在线节点”也才 3 个。**

这个时候，分区 A 里的选举是基于它们**能看到的 3 个节点**计算的。候选者 1 只要能拿到这 3 个节点中的 2 票（含自己），就会误以为自己“过半数”了，从而当选 Leader，执行故障转移。

**这就叫“小集群里的多数派”。** Sentinel 为了防止完全僵死，允许少数派在满足自身分区内多数时完成选举，但这会带来“脑裂”风险（另一个分区也选了 Leader）。
...
- 假设现在是纪元 5，选举刚失败。A 和 B 都准备下一轮。
  1. **随机等待阶段**
     - Sentinel A 的随机定时器抽到了一个较短的时间：**50 毫秒**。
     - Sentinel B 的随机定时器抽到了一个较长的时间：**200 毫秒**。
  2. **A 先发制人 (在 50ms 时)**
     - A 的定时器率先到期。
     - A 立刻行动起来：将纪元 **递增到 6**，然后向包括 B 在内的所有其他哨兵**发送投票请求**（“纪元 6，请投给我 A”）。
     - 此时，B 还在沉睡中。
  3. **B 被唤醒并投票 (在 50ms ~ 200ms 之间)**
     - B 收到了 A 的投票请求。
     - B 检查自己的状态：我现在的纪元是 5，请求纪元是 6，这个请求是未来的，有效。而且我在纪元 6 里还没投过票。
     - 根据“先到先得”原则，B 更新自己的纪元到 6，**并把票投给了 A**。然后 B 继续沉睡。
  4. **B 自己的定时器到期 (在 200ms 时)**
     - B 终于醒来，准备发起自己的选举。
     - **但就在它想行动的那一刻，它发现：** 自己的纪元已经是 6 了，并且在纪元 6 里，它已经投过票了（投给了 A）。
     - 根据规则，每个哨兵在一个纪元里只能投一次票。所以，**B 不能再成为候选者，也无法再发起选举了**。
- A 拿到 2 票，当选。
...
这种情况在 Sentinel 集群里发生的概率相对较高。因为 Sentinel 判断主节点故障，不是直接问主节点“你还活着吗”，而是**所有哨兵自己去 Ping 主节点**，然后互相通信确认。

### 标准的Raft 协议如何处理这个场景:o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:

标准的 Raft 协议（etcd、Consul 等）通过 Leader 的主动“发声”机制，极大降低了这种脑裂风险。

在 Raft 中，Leader 必须周期性向所有 Follower 发送心跳，维持自己的领导地位。

- **隔离在机房A的旧主**：网络中断后，它无法把心跳发给多数派节点。超过心跳超时时间后，旧 Leader 会因为收不到多数派的确认而**主动、立即、无条件地下台**，变成一个普通节点，拒绝服务客户端的写请求。
- **机房B的多数派**：超时后，它们会发起选举，选出一个合法的新 Leader。

Raft 的这个机制确保了旧 Leader 在脑裂发生时，会**主动让位**，避免了冲突。
...
断线太久：与主节点断连时间超过 down-after-milliseconds * 10 的。

主观下线：被 Sentinel 标记为 SDOWN 的。

回复太慢：最近 5 秒内没有回复过 INFO 的。
这一关筛掉了所有“可能不靠谱”的。

第二道关：按优先级排序
剩下的“健康”从节点，按以下顺序逐个对比：

slave-priority (优先级)：配置文件中设置的，数字越小，优先级越高。

slave_repl_offset (复制偏移量)：偏移量越大，数据越新，越优先。

runid (运行ID)：如果前面都一样，就选 runid 字典序更小的，保证确定性。

Leader 就选这个排序列表的第一个，把它晋升为新主。
...
>
> 减轻手段：
> - `min-replicas-to-write` + `min-replicas-max-lag`：减少写入只在主库而没到从库的情况:o::o::o::o::o::o::o::o:
...
#### 3.5.2 减轻脑裂损失的配置:rocket::rocket::rocket::rocket::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
...
**面试追问：客户端在收到切换通知之前，发往旧主的写请求怎么处理？**:o::o::o::o:
...
### `sentinel parallel-syncs` 为什么默认是 1:o::o::o::o::o::o::o::o::o::o::o::o::o:
