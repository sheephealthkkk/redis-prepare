...
- **只输出每种类型的最大 key，不输出第二大、第三大**——如果前 10 大 key 你都需要 → 自己写 SCAN 脚本
...
**拆分原则：**
- Hash / Set / ZSet：按 ID 取模 / CRC32 哈希分桶
- 时间序列数据：按日期/小时拆分 → 旧 key 自然过期清理
- String：按业务维度拆（如分页、分段）
