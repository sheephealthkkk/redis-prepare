...
## 5大核心应用：:o:分布式锁，限流，消息队列，延时队列，分布式session:o:
...
**致命问题：** 拿到锁后如果服务宕机，`unlock` 代码没机会执行 → key 永久存在 → **死锁**。:rocket::rocket::o:
...
**致命问题：SETNX 和 EXPIRE 是两条独立命令，中间可能宕机。** 一个命令执行完，第二个命令还没发出去，进程被 kill -9 → 锁设置了但没有过期时间 → 死锁。:rocket::o::o::o:
...
**新问题——误删别人的锁：**过期之后解锁，解的锁不是自己的锁:rocket::rocket::o::o::o::o::o::o::o::o::o::o::o:
...
#### V1-V4 还没解决的问题：过期时间到底设多长？:rocket::rocket::rocket::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
...
**原因二：验证身份。** 解锁时 `hexists` 判断 "解锁者是否就是加锁者"，避免误删。:o::o::o::o::o::o::o::o::o:
...
看门狗是 Redisson 的杀手级特性。它解决的核心矛盾是：:o::o::o::o::o::o:**"不设过期怕死锁，设了过期怕没执行完"**。:rocket::rocket::rocket:
...
### 1.4 理解":o::o:主从切换为什么导致锁失效":o:——红锁的前置知识
...
**为什么必须"过半"？** 这是分布式共识的基本法则。如果两个客户端同时抢锁，它们不可能同时获得大多数节点的成功——因为每个节点只能被一个客户端 SET NX 成功。这是靠 NX 条件保证的。:rocket::rocket::rocket::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:

**为什么总耗时必须 < TTL？** 因为从第一个节点 SET 成功到最后一个节点 SET 成功之间拖了时间，锁的实际剩余有效时间变短了。如果总耗时接近或超过 TTL，说明这条锁实际上已经没保护了。:rocket::o::o::o::o::o::o::o:
...
**NTP 系统时间跳变**，简单说就是系统时钟被瞬间、大幅度地调整到了另一个时间，而不是平滑地慢慢修正。在 NTP 术语里，这种瞬间调整叫**步进（step）**，和缓慢微调（slew）相对。

### 两种时间调整方式

正常情况下，NTP 会用 **slew（渐进）** 的方式，通过加快或减慢系统时钟的频率（比如每秒调整几毫秒），让本地时间不知不觉地追上正确时间。
但当本地时间与标准时间的**偏差过大**时，慢调太耗时，NTP 就会直接**步进**——瞬间把系统时钟拨到正确值，这就造成了“时间跳变”。
...
- **系统启动时时钟严重不准**（最常见）
  比如硬件时钟错误、主板电池没电、新机器 BIOS 时间还停留在出厂值，导致和真实时间差了好几个小时甚至几年。
- **长时间未同步**
  如果 NTP 服务停了很久，积累的偏差超过阈值，恢复时就会跳变。
- **强制步进操作**
  手动执行 `ntpdate`、`ntpd -g` 或 `chronyc makestep` 等命令，主动触发一次性步进。
- **NTP 守护进程的默认行为**
  以 `ntpd` 为例，启动时如果偏差超过 128 ms，它默认会直接步进一次。之后运行中若偏差再超过 **panic 阈值**（默认 1000 秒），`ntpd` 反而会**退出**，根本不会去步进，以免造成巨大跳变。如果想允许大偏差时步进，需要显式配置。

### 时间跳变会造成什么问题？

瞬间的时间变化（尤其是向后跳变）会打乱依赖连续时间的程序，比如：

- **日志时间戳乱序**，影响排错和审计。
- **定时任务（cron）** 可能被跳过一次，或在短时间内重复执行。
- **文件时间戳异常**，导致 `make` 等工具错误判断依赖、增量备份遗漏。
- **数据库事务和复制** 出现异常时间戳，严重时破坏一致性。
- **分布式系统心跳超时**、证书有效期误判等。
...
**Raft 的核心思想：** 先选出一个 Leader（领导），所有写操作必须经过 Leader，Leader 把决策同步给 Follower（追随者），过半确认才提交。:o::o::o::o::o::o::o::o::o::o::o:
...
**Raft 的锁实现（etcd 用 Raft）：** 锁 = 一个 KV 条目。加锁 = 把这个 KV 条目通过 Raft 提交到半数以上节点。**Leader 宕机了不会丢锁**——新 Leader 包含所有已提交的条目（包括锁）。:o::o::o::o::o::o::o::o:
...
| **共识算法** | 无（异步复制） | 无（客户端投票）:o::o: | ZAB（类 Raft） | Raft |
...
自己写的mq里面，broker就是redis这种做法，nameserver是etcd的做法。:o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
...
**核心矛盾：** :o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:单机限流感知不到全局:rocket::rocket:。集群的总流量是各实例流量的总和，但单个实例不知道别的实例在做什么。
...
**为什么 Redis？** :o::o::o::o::o:因为 Redis 是单线程执行命令的，多台服务器同时 INCR 同一个 key → Redis 排队串行执行 → 计数精确、没有竞态。如果三台服务器各自在本地内存中做 `AtomicInteger.incrementAndGet()`，它们是三个不同的对象，无法加起来。
...
滑动窗口已经很好了——没有边界漏洞，统计平滑。但它有一个缺点：**不允许突发。**:o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
...
**为什么桶容量 = rate（10）而不是更大？**:o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
...
把窗口切小、把桶容量设低，表面上都能控制“最多允许多少请求”，但它们处理**空闲时间**和**突发后恢复**的方式，有本质区别。正是这个区别，决定了谁更能优雅地应对“流量突刺”。

### 1. 核心差异：空闲时能否“攒信用”

- **令牌桶**：可以攒令牌。系统空闲时，令牌会攒在桶里（最多到容量上限）。这就是它应对突发的资本：闲久了，可以瞬间爆发一大波流量。
- **滑动窗口**：不攒信用。它只看过去一个窗口内有多少请求。无论你闲了多久，当前窗口内允许通过的请求数上限是固定的，不会增加。

### 2. 关键区别在突发“之后”

你提到“令牌数设小点”和“窗口设小点”，参数上是能凑出相近的平均速率和最大突发，但突发流量过去后，情况就完全不同了。

假设我们都要求：**平均 10次/秒，最大突发 10次**。

- **方案A：令牌桶（容量=10，速率=10/秒）**
- **方案B：精确滑动窗口（窗口=1秒，上限=10次）**

在**瞬间爆发10个请求**的那一刻，两者行为一致：全部放行。

**但从第11个请求开始，差别就出现了：**

- **令牌桶：平滑恢复**
  桶里的令牌瞬间被掏空。接下来，令牌以“10个/秒”的速度生成，即每0.1秒产一个。那么，在突发过去0.1秒后，桶里就有1个令牌，可以再放行一个请求。系统很快恢复了部分处理能力。

- **精确滑动窗口：强制“惩罚期”**
  窗口内记录着刚刚那10个请求的时间戳。接下来的1秒内，无论你怎么算，这10个请求都死死卡在窗口里。**在第一个请求的时间戳移出窗口（即1秒后）之前，计数始终为10，所有新请求都会被无情拒绝。** 系统要完全卡死1秒，然后瞬间释放10个名额。:o::o:

  你看，令牌桶让你在突发后“缓缓喘口气”，而滑动窗口让你“直接憋气一分钟”。对于等待响应的客户端，前者可能只是部分延迟增加，后者则是整整1秒的完全不可用。:o::o::o::o::o::o::o::o::o::o::o::o::o:
...
- **固定窗口计数器 (Fixed Window)**
  最容易想到：窗口期内计数，超了就等下一个窗口。问题是“临界突刺”。
- **滑动窗口日志 (Sliding Log)**
  精确但昂贵：记录每个请求时间戳，每次清理过期数据并计数。绝对平滑，但内存和计算开销大。
- **滑动窗口计数器 (Sliding Window Counter)**
  前面的折中版：把窗口分成小段，按当前窗口计数 + 上一个窗口的加权，估算真实计数。既平滑又省内存。

**2. 从容应对突发的“桶”家族**

- **漏桶 (Leaky Bucket)**
  你之前没提，但它和令牌桶是双子星。漏桶是**强行平滑输出**：请求先入桶，桶以固定速率漏水（处理）。桶满了就拒绝。它完全削平突发，不管你闲不闲，输出始终匀速。适合保护脆弱下游。
- **令牌桶 (Token Bucket)**
  允许攒信用，输出可带突发。适合大多数互联网服务。

**3. 更高级的“自适应”与“分布式”思路**

- **自适应限流 (Adaptive/Intelligent)**
  不设固定 QPS，而是根据系统自身负载动态调整。例如：
  - **BBR 思想**：像 TCP BBR 那样，根据 RTT 和吞吐量探测瓶颈带宽，动态调整发送速率。
  - **梯度限流**：根据系统 CPU、请求延迟等指标，用类似梯度下降的方法不断试出当前最大能承载的 QPS。
  - **激流 (Bolt-on) 或 Sentinel 自适应**：并发请求的 RT 超过阈值就开始减少通过量。
- **分布式限流**
  当服务是集群时，需要全局统一计数，这就催生了基于 Redis 原子操作、或更复杂的分布式共识算法来实现令牌桶/滑动窗口。这不是新算法本身，而是将单机算法分布式化的**工程模式**。
- **许可证/信号量**
  控制最大并发数而非速率。比如设置“最多同时处理 100 个请求”，处理完一个才释放一个。这是并发控制，但常和限流配合使用。

**这些思路的本质，都是在“公平性”、“突发容忍度”、“实现精度”和“系统开销”之间做权衡。**
...
### 3.2 Redis Stream——带 ACK 的轻量级消息队列:o::o::o::o::o:
...
### 3.3 PEL——消息可靠性的核心:o::o::o::o::o::o::o::o::o::o::o::o::o:
...
**为什么这样设计？** ZSet 擅长"按时间排序暂存"（score 天然是时间索引），MQ 擅长"可靠投递 + ACK + 消费组"。两者各取所长。:o::o::o::o::o::o::o::o::o::o::o::o:mq内置的延迟都很拉，rocketmq有内置的时间延迟，但是如果如果要自定义延时多久呢，kafka，rabbitmq更是没有这个功能笑死了:o::o::o::o::o::o::o::o:
...
#### 4.2 后端节点变化导致重映射:o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o::o:
...
- **仅影响相邻节点之间的一小段**。:o::o::o::o::o::o::o::o::o::o::o:
